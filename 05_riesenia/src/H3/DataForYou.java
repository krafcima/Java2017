...
public class DataForYou {
	...
	public static String najcastejsie(){
		HashMap<String,Integer> mapa =new HashMap<String,Integer>();
		for (int i = 0; i < spoluziaci.length; i++) {
			int pocet= 1;
			if(mapa.get(spoluziaci[i])!=null){
				pocet = mapa.get(spoluziaci[i]) +1;
			} 
			mapa.put(spoluziaci[i],pocet);
		}
		
		String najcastejsie=null;
		int pocetNajcastejsie =0;
//		for(String meno : mapa.keySet()){
//			if(mapa.get(meno) > pocetNajcastejsie ){
//				najcastejsie = meno;
//				pocetNajcastejsie =mapa.get(meno);
//			}
//		}
		
		for(Map.Entry<String, Integer> dvojica : mapa.entrySet()){
			if(dvojica.getValue() > pocetNajcastejsie){
				najcastejsie = dvojica.getKey();
				pocetNajcastejsie = dvojica.getValue();
			}
		}
		
		System.out.println(pocetNajcastejsie);
		return najcastejsie;
	}
	 
	
	public static int pocetMien() {
		Set<String> mnozina = new TreeSet<String>();
		for (int i = 0; i < spoluziaci.length; i++) {
			mnozina.add(spoluziaci[i]);
		}
		System.out.println(mnozina);
		return mnozina.size();
	}
	
	public static void menaPodlaDlzky(){
		Set<String> mena = new TreeSet<String>(new Comparator<String>() {
